...
// Limit FPS to reduce CPU/GPU usage when idle
let lastFrameTime = 0;
let targetFPS = 30; // Default to 30 FPS
...
const adjustFPS = (isPlaying) => {
  targetFPS = isPlaying ? 60 : 15; // Higher FPS when playing music
};
...
function updateUI(data) {
  ...
  rafId = requestAnimationFrame(() => {
    rafId = null;
    const updateData = pendingUpdate || data;
    pendingUpdate = null;
    
    const { media, system } = updateData;
    currentMedia = media;
  ...
  if (system) {
    ...
    if (cpuNameDetail)
      cpuNameDetail.textContent = system.cpuName || "PROCESSOR";

    if (ramUsageDetail) ramUsageDetail.textContent = `${system.ram}%`;
    if (ramInfoDetail)
      ramInfoDetail.textContent = `${system.ramUsed} / ${system.ramTotal} GB`;
  ...
  if (media) {
    ...
    if (media.thumbnail) {
      if (albumImg.src !== media.thumbnail) {
...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  if (savedColor) {
    ...
    const savedDot = document.querySelector(
      `.color-dot[data-color="${savedColor}"]`,
    );
...
// Idle detection to pause animations and reduce CPU usage
